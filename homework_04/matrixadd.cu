...
void MatrixAddOnDevice(const Matrix M, const float alpha, const Matrix N, const float beta, Matrix P)
{
   ...
   errord = cudaEventCreate(&startin);
   if (errord != cudaSuccess)
   {
       fprintf(stderr, "Failed to create start event (error code %s)!\n", cudaGetErrorString(errord));
       exit(EXIT_FAILURE);
   }
   errord = cudaEventCreate(&stopin);
   if (errord != cudaSuccess)
   {
       fprintf(stderr, "Failed to create stop event (error code %s)!\n", cudaGetErrorString(errord));
       exit(EXIT_FAILURE);
   }
   errord = cudaEventCreate(&startex);
   if (errord != cudaSuccess)
   {
       fprintf(stderr, "Failed to create start event (error code %s)!\n", cudaGetErrorString(errord));
       exit(EXIT_FAILURE);
   }
   errord = cudaEventCreate(&stopex);
   if (errord != cudaSuccess)
   {
       fprintf(stderr, "Failed to create stop event (error code %s)!\n", cudaGetErrorString(errord));
       exit(EXIT_FAILURE);
   ...
   // Record the start event
   errord = cudaEventRecord(startin, NULL);
   if (errord != cudaSuccess)
   {
       fprintf(stderr, "Failed to record start event (error code %s)!\n", cudaGetErrorString(errord));
       exit(EXIT_FAILURE);
	...
	/* We are told array size is fixed at 4096
	 * Were that not the case, this should be changed to be more
	 * general based on the matrices, to optimze performance.
	 * 64*64*4096 = 4096*4096
	 */
	dim3 DimGrid(64,4096,1);
	dim3 DimBlock(64);
	//dim3 DimGrid(4,4096,1);
	//dim3 DimBlock(1024);

   // Record the start event
   errord = cudaEventRecord(startex, NULL);
   if (errord != cudaSuccess)
   {
       fprintf(stderr, "Failed to record start event (error code %s)!\n", cudaGetErrorString(errord));
       exit(EXIT_FAILURE);
	...
	printf("executing \n");
	MatrixAddKernel <<<DimGrid, DimBlock>>> (dM.elements, alpha, dN.elements, beta, dP.elements, M.width*M.height);
   ...
   // Record the stop event
    errord = cudaEventRecord(stopex, NULL);
    if (errord != cudaSuccess)
    {
        fprintf(stderr, "Failed to record stop event (error code %s)!\n", cudaGetErrorString(errord));
        exit(EXIT_FAILURE);
    ...
    // Wait for the stop event to complete
    errord = cudaEventSynchronize(stopex);
    if (errord != cudaSuccess)
    {
        fprintf(stderr, "Failed to synchronize on the stop event (error code %s)!\n", cudaGetErrorString(errord));
        exit(EXIT_FAILURE);
    ...
    errord = cudaEventElapsedTime(&exclusive, startex, stopex);
    if (errord != cudaSuccess)
    {
        fprintf(stderr, "Failed to get time elapsed between events (error code %s)!\n", cudaGetErrorString(errord));
        exit(EXIT_FAILURE);
    ...
    //**** Record the inclusive stop event ****
    errord = cudaEventRecord(stopin, NULL);
    if (errord != cudaSuccess)
    {
        fprintf(stderr, "Failed to record stop event (error code %s)!\n", cudaGetErrorString(errord));
        exit(EXIT_FAILURE);
    ...
    // Wait for the stop event to complete
    errord = cudaEventSynchronize(stopin);
    if (errord != cudaSuccess)
    {
        fprintf(stderr, "Failed to synchronize on the stop event (error code %s)!\n", cudaGetErrorString(errord));
        exit(EXIT_FAILURE);
    ...
    errord = cudaEventElapsedTime(&inclusive, startin, stopin);
    if (errord != cudaSuccess)
    {
        fprintf(stderr, "Failed to get time elapsed between events (error code %s)!\n", cudaGetErrorString(errord));
        exit(EXIT_FAILURE);
