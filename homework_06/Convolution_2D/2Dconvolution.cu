...
__global__ void ConvolutionKernel(Matrix M, Matrix N, Matrix P)
{
	...
	if (tidxy < KERNEL_SIZE*KERNEL_SIZE)
	{
   ...
   if (nx<(N.width) && ny<(N.height))
   {
	...
	//Make sure we don't go past bounds of what is defined
      int x0 = (nx < KERNEL_SIZE/2)? (KERNEL_SIZE/2-nx) : 0;
      int x1 = (nx > (N.width-KERNEL_SIZE/2-1))? (N.width-nx+KERNEL_SIZE/2) : KERNEL_SIZE;
      int y0 = (ny < KERNEL_SIZE/2)? (KERNEL_SIZE/2-ny) : 0;
      int y1 = (ny > (N.height-KERNEL_SIZE/2-1))? (N.height-ny+KERNEL_SIZE/2) : KERNEL_SIZE;
	...
	//Perform the convolution using M and N
      for(int i=x0; i<x1; i++)
      {
         for(int j=y0; j<y1; j++)
         {
            sum = sum + sM[i+j*M.width]*N.elements[(i+nx-KERNEL_SIZE/2) + (ny+j-KERNEL_SIZE/2)*N.width];
         }
      }
