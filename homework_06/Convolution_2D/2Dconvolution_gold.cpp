...
computeGold(float* C, const float* A, const float* B, unsigned int hB, unsigned int wB)
{
	...
	for (unsigned int i = 0; i < hB; ++i){
        for (unsigned int j = 0; j < wB; ++j) {
			...
			//  matrix edges
			unsigned int mbegin = (i < KERNEL_SIZE/2)? KERNEL_SIZE/2 - i : 0;
			unsigned int mend = (i > (hB - KERNEL_SIZE/2 -1))?
									hB - i + KERNEL_SIZE/2 : KERNEL_SIZE;
			unsigned int nbegin = (j < KERNEL_SIZE/2)? KERNEL_SIZE/2 - j : 0;
			unsigned int nend = (j > (wB - KERNEL_SIZE/2 -1))?
									(wB-j) + KERNEL_SIZE/2 : KERNEL_SIZE;
			...
			for(unsigned int m = mbegin; m < mend; ++m) {
				for(unsigned int n = nbegin; n < nend; n++) {
					sum += A[m * KERNEL_SIZE + n] * 
							B[wB*(i + m - KERNEL_SIZE/2) + (j+n - KERNEL_SIZE/2)];
				}
			}
			// store the result
			C[i*wB + j] = (float)sum;
