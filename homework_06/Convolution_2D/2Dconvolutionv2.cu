...
__constant__ float* cM;

////////////////////////////////////////////////////////////////////////////////
// Matrix multiplication kernel thread specification
////////////////////////////////////////////////////////////////////////////////
__global__ void ConvolutionKernel(float* cM, Matrix N, Matrix P)
{
   ...
   if (nx<(N.width) && ny<(N.height))
   {
      ...
      for(int i=x0; i<x1; i++)
      {
         for(int j=y0; j<y1; j++)
         {
            sum = sum + cM[i+j*KERNEL_SIZE]*N.elements[(i+nx-2) + (ny+j-2)*N.width];
         }
      }
...
void ConvolutionOnDevice(const Matrix M, const Matrix N, Matrix P)
{
    ...
    cudaMalloc((void**)&cM, M.width * M.height * sizeof(float));
    cudaMemcpyToSymbol(cM,M.elements,sizeof(float)*KERNEL_SIZE*KERNEL_SIZE);
	...
	ConvolutionKernel<<<dimGrid,dimBlock>>>(cM,Nd,Pd);
